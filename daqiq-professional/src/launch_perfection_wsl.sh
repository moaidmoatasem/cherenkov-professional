...
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║  🚀 DAQIQ PERFECTION LAUNCH - WSL2 OPTIMIZED                ║"
echo "╚══════════════════════════════════════════════════════════════╝"
echo ""
...
status "Building DAQIQ Docker image..."
if docker build -f Dockerfile.optimized -t daqiq:perfection . --no-cache 2>&1 | tail -20; then
    success "Docker image built successfully"
else
    warning "Docker build had warnings - checking if image exists..."
    if docker images | grep -q "daqiq"; then
...
docker network create daqiq-network 2>/dev/null || success "Network already exists"

status "Creating volumes..."
docker volume create ollama-data 2>/dev/null || success "Ollama volume exists"
docker volume create daqiq-output 2>/dev/null || success "Output volume exists"
...
if docker ps -a | grep -q daqiq-ollama; then
    status "Removing old Ollama container..."
    docker rm -f daqiq-ollama
fi

status "Starting Ollama container..."
docker run -d \
    --name daqiq-ollama \
    --network daqiq-network \
    -p 11434:11434 \
...
docker exec daqiq-ollama ollama pull qwen2.5:3b &
OLLAMA_PID=$!

# Step 5: Launch DAQIQ Scanner (while Ollama pulls model)
echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "STEP 5: LAUNCH DAQIQ SCANNER"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""

# Remove old container
if docker ps -a | grep -q daqiq-scanner; then
    docker rm -f daqiq-scanner
fi

status "Starting DAQIQ scanner container..."
docker run -d \
    --name daqiq-scanner \
    --network daqiq-network \
    -v "$(pwd)/output:/app/output" \
    -v "$(pwd)/logs:/app/logs" \
    -e OLLAMA_BASE_URL=http://daqiq-ollama:11434 \
...
# Remove old container
if docker ps -a | grep -q daqiq-dashboard; then
    docker rm -f daqiq-dashboard
fi

status "Starting web dashboard..."
docker run -d \
    --name daqiq-dashboard \
    --network daqiq-network \
    -p 5000:5000 \
    -v "$(pwd)/output:/app/output" \
...
# Check Ollama
if docker exec daqiq-ollama ollama list &> /dev/null; then
...
# Check Scanner
if docker exec daqiq-scanner python --version &> /dev/null; then
...
docker ps --filter "name=daqiq-" --format "table {{.Names}}\t{{.Status}}\t{{.Ports}}"
...
docker stats --no-stream --format "table {{.Name}}\t{{.CPUPerc}}\t{{.MemUsage}}\t{{.MemPerc}}" \
    daqiq-ollama daqiq-scanner daqiq-dashboard 2>/dev/null

echo ""
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║  ✅ DAQIQ PERFECTION - FULLY OPERATIONAL                    ║"
echo "╚══════════════════════════════════════════════════════════════╝"
echo ""
...
echo "📋 Quick Commands:"
echo "  • Test scan:"
echo "    docker exec daqiq-scanner python daqiq_simple_scanner.py https://example.com"
echo ""
echo "  • View logs:"
echo "    docker logs -f daqiq-scanner"
echo "    docker logs -f daqiq-dashboard"
echo ""
echo "  • Monitor resources:"
echo "    watch -n 2 'docker stats --no-stream daqiq-*'"
echo ""
echo "  • Stop all:"
echo "    docker stop daqiq-ollama daqiq-scanner daqiq-dashboard"
echo ""
echo "  • Clean up:"
echo "    docker rm daqiq-ollama daqiq-scanner daqiq-dashboard"
echo ""
