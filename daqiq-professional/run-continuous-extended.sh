...
echo "🔥 CONTINUOUS EXTENDED DEVELOPMENT"
echo "===================================="
echo ""
echo "⏰ Started: $(date)"
echo "🎯 Target: 60+ scanners from extended-overnight-tasks.yaml"
echo ""
...
# Parse the YAML and generate scanners one by one
cat > scripts/process_extended_tasks.py << 'PYTHON'
#!/usr/bin/env python3
"""
...
# Make executable
chmod +x scripts/process_extended_tasks.py

# Run it in background
nohup python3 scripts/process_extended_tasks.py \
    > logs/overnight/extended-continuous-$(date +%Y%m%d-%H%M%S).log 2>&1 &

EXTENDED_PID=$!
echo $EXTENDED_PID > /tmp/daqiq-extended-pid.pid

echo "✅ Extended continuous development started!"
echo "   PID: $EXTENDED_PID"
echo "   Log: logs/overnight/extended-continuous-$(date +%Y%m%d-%H%M%S).log"
echo ""
echo "📊 Monitor: tail -f logs/overnight/extended-continuous-*.log"
echo "🛑 Stop: kill $EXTENDED_PID"
echo ""
